...
from caid_models import CAiD_UNet,CAiD_MoCo,MoCo
...
parser.add_argument('--mode', default='caid', type=str,
                     help='caid|id for CAiD or Instance Discrimination')
...
parser.add_argument('--exp_name', default="CAiD_moco", type=str,help='experiment name')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.mode.lower() == "id":
        ...
        model = CAiD_MoCo(CAiD_UNet, args.moco_dim, args.moco_k, args.moco_m, args.moco_t, args.mlp, backbone=args.arch, encoder_weights=args.encoder_weights, activation=args.activate)
    print(model)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, nce_criterion, mse_criterion, optimizer, epoch, args)
        counter = validate(valid_loader, model, nce_criterion, mse_criterion, epoch, args)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            torch.save({
                ...
                'best_loss':best_loss
            }, os.path.join(args.checkpoint_dir,'current_checkpoint.pth'))

    if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
        if args.mode.lower() == "id":
            torch.save(model.module.encoder_q.state_dict(), os.path.join(args.checkpoint_dir, 'checkpoint.pth'))
        elif args.mode.lower() == "caid":
            torch.save(model.module.encoder_q.backbone.state_dict(), os.path.join(args.checkpoint_dir, 'checkpoint.pth'))
